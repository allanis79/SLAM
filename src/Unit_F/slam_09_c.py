...
class ExtendedKalmanFilterSLAM:
    ...
    def predict(self, control):
        ...
        if N > 0:
	        ...
	        Z1 = zeros((N*2,3))
	        I = eye(N*2)
	        Z2 = concatenate((G3,Z),axis =1)
	        Z3 = concatenate((Z1,I),axis =1)
	        G3_f = concatenate((Z2,Z3))
	        Z_4 = concatenate((R3,Z), axis = 1)
	        Z_5 = zeros((N*2,N*2+3))
	        R3_f = concatenate((Z_4,Z_5))
	      
	        h = dot(self.covariance, G3_f.T)

	        self.covariance = dot(G3_f, h) + R3_f
	        
	        xx = self.get_landmarks()
	        
	        
	        self.state = self.g(self.state, control, self.robot_width)
	        self.state = concatenate((self.state,array([xx[0][0]]),array([xx[0][1]])))

        else:
	    	self.covariance = dot(G3, dot(self.covariance, G3.T)) + R3
	        self.state = self.g(self.state, control, self.robot_width)
    ...
    def add_landmark_to_state(self, initial_coords):
        ...
        N = self.number_of_landmarks +1
        
        self.state = concatenate((self.state,array([initial_coords[0]]),array([initial_coords[1]])))
        dia = diag([1e10]*N*2)
        Z = zeros((N*2+1,N*2))
        Z1 = zeros((N*2,N*2+1))
        Z2 = concatenate((self.covariance,Z),axis =1)
        Z3 = concatenate((Z1,dia),axis =1)
     
        self.covariance = concatenate((Z2,Z3))
    ...
    def correct(self, measurement, landmark_index):
        ...
        constant_matrix = H3[0:2,0:2] * -1
    

        final_matrix = zeros((2,3+2*self.number_of_landmarks))
       

        final_matrix[0:3,0:3] = H3
  
        final_matrix[0][3+2*landmark_index : 3+2*landmark_index+2] = constant_matrix[0]
        final_matrix[1][3+2*landmark_index : 3+2*landmark_index+2] = constant_matrix[1]
        H = final_matrix
