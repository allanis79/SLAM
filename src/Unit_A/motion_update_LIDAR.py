...
def filter_step(old_pose, motor_ticks, ticks_to_mm, robot_width,
    ...
    if motor_ticks[0] == motor_ticks[1]:
        ...
        theta = old_pose[2]
        x = (old_pose[0] + (motor_ticks[0] * ticks_to_mm)*cos(old_pose[2])) #+ scanner_displacement
        y = (old_pose[1] + (motor_ticks[1] * ticks_to_mm)*sin(old_pose[2])) #+ scanner_displacement
        ...
        X = old_pose[0] +scanner_displacement
        Y = old_pose[1] + scanner_displacement
        cx = X -((R+(robot_width/2))*sin(old_pose[2]))
        cy = Y+((R+(robot_width/2))*(cos(old_pose[2])))
